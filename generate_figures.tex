...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\begin{axis}[%
			...
			xtick={0.5,0.52,...,0.6},%
			ylabel=$\xi/\sigma$, ymin=0.5, ymax=2.1,%
			...
			%\addplot+[mark=none, forget plot, domain=0.49:0.58] {0.41158778*(1-x/0.59773962)^(-2.0/3.0)};
			%\addplot+[only marks, fill=white] table[x index=0, y index=2]{xi_phi.dat};
			\addplot+[mark=none, forget plot, domain=0.49:0.58] {0.18878182*(1-x/0.59773962)^(-2.0/3.0)};
			\addplot+[only marks, fill=black] table[x index=0, y index=3]{scale.xi};
			\addplot+[mark=none, dashed, forget plot, domain=0.49:0.58] {0.194752*(1-x/0.59773962)^(-2.0/3.0)};
			\addplot+[only marks, fill=white] table[x index=0, y index=5]{scale.xi};
...
\begin{figure}
	\begin{tikzpicture}[
		lab/.style={text height=0.8em, text depth=0.2em, font=\Large\bfseries}%
		]%
...
\begin{figure}
	...
	\matrix[%
		above left=0.01\textwidth of X.south east, fill=gray!25!white, semitransparent, text opacity=1, font=\footnotesize,% 
...
%	\node[right=0.01\textwidth] at (one_mrco.east) {$Q_6=Q_6^*$};
	...
	\end{tikzpicture}
	\caption{\textbf{Computer reconstruction from confocal microscopy coordinates in our most deeply supercooled sample.} Depth is $\sim 12\sigma$. Only particles of interest and their neighbours are displayed. Each particle is plotted with its real radius. \textbf{a} Sub-critical crystal nuclei. \textbf{b,} A typical configuration of bond ordered particles. Two icosahedral particle are shown in the same shade if they belong to the same cluster. If a particle is neighbouring both kind of structures, it is displayed as icosahedral. \textbf{c,} Crystal-like particles alone (the order parameter was averaged over $t^{dh}/2$). \textbf{d,} Slow particles (see text). Due to particles going in and out of the field of view, the edges of \textbf{c} and \textbf{d} were not accurate and have been removed.}
	\label{fig:3D}
\end{figure}
\tikzset{external/force remake=false}
\begin{figure}
