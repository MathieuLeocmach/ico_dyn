...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\begin{axis}[%
			...
			\addplot+[only marks, mark=square] table[x index=0, y expr=\thisrowno{5}/0.216]{scale.xi};
			\legend{$\xi_6$ structural, $\xi_4$ dynamical};
		\end{axis}
	\end{tikzpicture}
	\caption{\textbf{Dynamics of the system.} {\bf a,} Decay of the self-intermediate scattering function computed from the positions for several volume fractions. Lines are stretched exponential fit from which is extracted the structural relaxation time $\tau_\alpha$ {\bf b,} $\tau_\alpha$ and the characteristic time of the dynamic heterogeneity $t^\text{dh}$ scaled by the Brownian time $\tau_B$ as a function of $\phi$. The solid curve is the VFT fit of $\tau_\alpha$ ($\phi_0=0.600$, $D=0.328$). Inset: Dynamical ($\xi_4$) and structural correlation length ($\xi_6$) scaled by their respective bare correlation length, respectively $\xi_{4,0}=0.206\sigma$ and $\xi_{6,0}=0.126\sigma$. The curve is the power-law given in the text.}
	\label{fig:vft}
\end{figure}
...
\begin{figure}
	\begin{tikzpicture}[
		...
		\end{groupplot}
		%color scales by hand
		\matrix (l3) [right=of w6Q6 c2r1.right of east, matrix of nodes, minimum width=1em, minimum height=2em, inner sep=0, draw=black]
		{
			|[fill=black]| \\ 
			|[fill=black!75!white]| \\
			|[fill=black!50!white]| \\
...
\begin{figure}
	...
	\matrix[%
		above left=0.01\textwidth of X.south east, font=\footnotesize,% 
	...
	\end{tikzpicture}
	\caption{\textbf{Computer reconstruction from confocal microscopy coordinates in our most deeply supercooled sample.} Depth is $\sim 12\sigma$. Only particles of interest and their neighbours are displayed. Each particle is plotted with its real radius. \textbf{a} Particles with more than $7$ crystalline bonds. \textbf{b,} A typical configuration of bond ordered particles. Two icosahedral particle are shown in the same shade if they belong to the same cluster. If a particle is neighbouring both kind of structures, it is displayed as icosahedral. \textbf{c,} Crystal-like particles alone (the order parameter was averaged over $t^\text{dh}/2$). \textbf{d,} Slow particles (see text). Due to particles going in and out of the field of view, the edges of \textbf{c} and \textbf{d} were not accurate and have been removed.}
	\label{fig:3D}
\end{figure}
\tikzset{external/force remake=false}
\begin{figure}
...
\begin{figure}
	...
	\begin{loglogaxis}[%
		...
		\addplot+[fitc] {0.634/(1+0.703^2*x^2)};
		\addplot+[only marks] file {LS3954.S4};
		\addplot+[fitc] {0.744/(1+0.804^2*x^2)};
		\addplot+[only marks] file {LS4446.S4};
		\addplot+[fitc] {0.778/(1+0.835^2*x^2)};
		\addplot+[only marks] file {LS4582.S4};
		\addplot+[fitc] {1.011/(1+1.017^2*x^2)};
		\addplot+[only marks] file {LS5079.S4};
		\addplot+[fitc] {2.950/(1+1.959^2*x^2)};
		\addplot+[only marks] file {go1.S4};
		\addlegendimage{legend image code/.code={\node[right] {$\phi\;\pm$};}};
		\legend{$0.497$, $0.535$, $0.540$, $0.555$, $0.575$, $0.003$};
		\node[lab] at (rel axis cs:0, 1) {a};
	\end{loglogaxis}
