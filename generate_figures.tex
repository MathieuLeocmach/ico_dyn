...
\begin{figure}
	...
	\node[above right=0.01\textwidth, pic3d] (mrco) {\includegraphics[width=0.48\textwidth]{X_ngb_mrco_go1.png}};
	\matrix[%
		matrix anchor=north, matrix of nodes, ampersand replacement=\&,%
	...
	\end{tikzpicture}
	\caption{\textbf{Particles with more than $7$ crystalline bonds and their neighbours.} Particles with $Q_6>Q_6^*$ and their neighbours are also plotted in \textbf{b} for comparison. Crystalline particles and their neighbours are always included in high $Q_6$ regions. However, the fluctuations of the crystal-like order cannot be reduced to sub-critical nucleation.}
	\label{fig:X_3D}
\end{figure}
