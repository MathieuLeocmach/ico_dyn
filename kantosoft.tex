...
\institute[U. Tokyo, IIS]{The University of Tokyo, Institute of Industrial science, Tanaka Lab}
\title[icosahedral and crystal-like order]{Roles of icosahedral and crystal-like order in hard spheres glass transition}
\author[M. Leocmach]{\textsc{Leocmach} Mathieu}
...
\begin{flushright}
	\begin{tikzpicture}[concept/.style={scale=0.8, circle, font=\small, align=center, draw=bg, fill=bg, text width=6em, inner sep=0em}]
		\node[concept] (root) {glass transition};
		\node[below right=of root, concept] (s) {slow down of dynamics};
		\node[below=of root, concept] (d) {growing \emph{dynamic} length scale};
		\node<2>[below left=of root, concept] (sta) {growing \emph{static} length scale};
		...
		\node[below right=0.15\textwidth and 0.3\textwidth of root, concept] (root2) {criticality};
		\node[below left=of root2, concept] (d2) {growing \emph{static} length scale};
...
\end{tikzpicture} &  %
\centering\begin{tikzpicture}[scale=0.12]
...
\begin{frame}{Particle tracking}
	...
	\end{itemize}
	\uncover<2->{\begin{center}\includegraphics[width=0.5\columnwidth]{comp2D3D_crop}\end{center}}
	\end{columns}
\end{frame}
