...
\begin{frame}{Multiscale tracking : Arbitrary size distribution}
	\begin{columns}
	\column{0.2\textwidth}
	{\footnotesize Crocker \& Grier}\\
	\includegraphics[height=0.3\textheight, trim=5mm 1cm 5mm 1cm, clip=true]{comp3D_monoscale_r35_crop}\\
	{\footnotesize Our algorithm}\\
	\includegraphics[height=0.3\textheight, trim=5mm 1cm 5mm 1cm, clip=true]{comp2D3D_crop}
	\column{0.8\textwidth}
	\begin{tikzpicture}
	\begin{axis}[%
		name=hist2,
...
\begin{frame}{Glassy dynamics}
	...
	\begin{tikzpicture}
		...
		\begin{axis}[%
			...
			\node[below right,font=\footnotesize, text width=0.2\textwidth] at (rel axis cs:0,1) {V.F.T. law \[\tau_\alpha=\tau_\alpha^0 e^{\frac{D\phi}{\phi_0-\phi}}\]};
		\end{axis}
	\end{tikzpicture}
\end{frame}
...
\begin{columns}
	...
	\begin{tikzpicture}
		\begin{loglogaxis}[%
			...
			\node[above right, font=\footnotesize, text width=0.2\textwidth] at (rel axis cs:0,0) {O.Z. fit \[\frac{\chi_4}{1-q^2\xi_4^2}\]};
		\end{loglogaxis}
		\pgfplotsset{cycle list name=black white}
		\tikzset{every mark/.append style={scale=1.2}}
		\begin{axis}[%
