...
\begin{figure}
	\begin{tikzpicture}[baseline]
		\begin{axis}[%
			...
			legend columns=3, reverse legend, legend style={font=\footnotesize, at={(1,1.03)}, anchor=south east}, %
	...
	\begin{tikzpicture}[baseline]
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Dynamics of the system. Top: Self intermediate scattering function computed from the positions. Bottom: Relaxation time $\tau_\alpha$ function of $\phi$. The line is the VFT fit. Inset: Dynamical and structural correlation length. Lines are power law fit.}
	\label{fig:vft}
\end{figure}
	...
	\begin{tikzpicture}
		...
		\nextgroupplot[%
			colorbar sampled, colorbar style={%
				samples=6, xlabel={per units of $w_6\cdot Q_6$}, xtick={ 0.20,0.4,0.6, 0.8},% 
				xticklabels={$10^{1}$, $10^{2}$, $10^{3}$, $10^{4}$},%
				},%
		...
		\node [right] at (axis cs:-0.00782,0.4) {\footnotesize $w_6^{dodec}$};
...
We establish that in our system the tendency toward icosahedral order is best monitored by $w_6$, whereas $Q_6$ is the natural crystal-like order parameter. The population of local structures in the $(w_6,Q_6)$-plane is shown in Fig.~\ref{fig:maps2}, which confirms that the two tendencies are present in supercooled liquid of hard spheres. In particular to our surprise, icosahedra are present even at relatively low volume fraction (Fig.~\ref{fig:maps2}d). Both tendencies become more pronounced with deeper supercooling (Fig.~\ref{fig:maps2}c). Furthermore Fig.~\ref{fig:maps2}c show clearly that icosahedral order and crystalline order are incompatible and frustrate one another.
...
\begin{figure}
	...
	\node[anchor=south, draw, inner sep=0] (all) {\includegraphics[width=0.8\columnwidth]{mrco_ico_scale_go1-0023.png}};
	\matrix[%
		matrix anchor=north, draw, font=\footnotesize,% 
		column 1/.style={text height=0.8em, text depth=0.2em},%
		column 2/.style={circle, shade, inner sep=0.01\columnwidth},%
		] (l)
	{
		\node{Icosahedra}; & \node[ball color=red!33!blue] (b2) {}; \\
		\node{Crystal-like}; & \node[ball color=green!66!black] {}; \\
	};
	\node[anchor=north west] at (all.south west) (ico) {\includegraphics[width=0.25\columnwidth]{one_ico.png}};
	\node at (ico.south){$w_6=w_6^*$};
	\node[anchor=north east] at (all.south east) (mrco) {\includegraphics[width=0.25\columnwidth]{one_mrco.png}};
	\node at (mrco.south) {$Q_6=Q_6^*$};
	\node[lab, anchor=north] at (all.north) {$\phi=0.575$};
	\end{tikzpicture}
	\caption{Computer reconstruction from confocal microscopy coordinates of a typical configuration in our most deeply supercooled sample. Depth is $\sim 12\sigma$. Only ordered particles and their neighbours are displayed. Each particle is plotted with its real radius. Two icosahedral particle are shown in the same shade if they belong to the same cluster. If a particle is neighbouring both kind of structures, it is displayed as icosahedral. Bottom: Example of structures at the respective threshold values: distorted icosahedron (left) and crystal-like cluster (right).}
	\label{fig:3D}
\end{figure}
