...
\setsansfont[Scale = MatchLowercase]{MEgalopolis Extra}
\setmonofont{TeX Gyre Cursor}
%\usepackage{xunicode, xltxtra}
%\usepackage[oldstylenums, largesmallcaps]{kpfonts}
%\usepackage[math]{anttor}

%\setsansfont[Ligatures=Rare]{MEgalopolis Extra}
%\setsansfont{grotesq}
...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{arrows,shapes,backgrounds, positioning, intersections, decorations.text, shapes.geometric, matrix, chains}
...
\let\Textsize\normalsize
\def\Head#1{\noindent\hbox to \hsize{\hfil{\Large\color{Maroon}\raggedright\fontspec[Ligatures=Rare]{MEgalopolis Extra}%
	\ifthenelse{\isempty{#1}}{}%
	{{\addfontfeatures{StylisticSet=6}bd}{#1}{\addfontfeatures{StylisticSet=6}gi}}}%
\bigskip}
}
\def\LHead#1{\noindent{\LARGE #1}\smallskip}
\def\Subhead#1{\noindent{\large\textsf{#1}}}
\def\Title#1{\noindent{\VeryHuge\color{Maroon}\raggedright\fontspec[Ligatures=Rare]{MEgalopolis Extra}\uppercase{#1}}}
...
\baselineskip=3\baselineskip \Title{Roles of icosahedral and crystal-like order in hard spheres glass transition}
\hfill{\LARGE\fontspec[Ligatures=Rare, StylisticSet=5]{MEgalopolis Extra} Confocal microscopy study}
\end{textblock}

\begin{textblock}{15}(0,1.5)
\LHead{Mathieu Leocmach \hspace{5cm} \texttt{mathieu@iis.u-tokyo.ac.jp}\\
Hajime Tanaka \hspace{5cm} \textsc{Institute of Industrial science, The University of Tokyo}}
\end{textblock}
...
\begin{textblock}{3}(0,7)
	...
	\textsc{\Large `Crystal' }is the symmetry breaking phase, avoided \emph{by definition} to get the supercooled liquid.
...
\begin{textblock}{2.875}(12,3.125)
	\Head{Conclusion}
	\Subhead{In hard spheres}
	\begin{itemize}
		\item Icosahedral order
		\begin{itemize}
			\item no role in slowing down
			\item probably a frustration role on the crystallisation
		\end{itemize}
		\item Crystal-like order
		\begin{itemize}
			\item governs slow dynamics
			\item critical-like fluctuations
		\end{itemize}
	\end{itemize}
	\Subhead{In general,} the structural origin of the dynamical arrest may be
	\begin{itemize}
		\item linked to the avoided first order transition (crystallisation)
		\item not linked to the condensation of the order of the liquid (icosahedral)
	\end{itemize}
