...
Figure~S\ref{fig:sizing}a shows the size distribution of the suspensions investigated in the main text. Once controlled for a solvent swelling of $23\%$ in radius, our \emph{in situ} measurements compare well with the size distribution obtained from scanning electron microscopy (\textsc{sem}) of the same dry colloids. However we note that our method finds relatively more small particles than the \textsc{sem}. A careful comparison between confocal images and algorithm output suggest that this is not an artefact: the objects detected as small ($\unit{1.5}{\micro\metre}$ to $\unit{3}{\micro\metre}$) do correspond to small spots (3-4 pixels width) in the confocal images. Thus we conclude that the relative lack of small particles in the \textsc{sem} measurement is probably due to bad sampling (the sizes of only $140$ particles were measured). 
...
In Fig.~M\ref{fig:vft}a we fit the self intermediate scattering function by the VFT law $\tau_\alpha=\tau_\alpha^0 \exp(D\phi/(\phi_0-\phi))$ ($D$ is the fragility index) to get the structural relaxation time $\tau_\alpha$ upon $\phi$. The non-Gaussianity of the dynamics is monitored by the kurtosis of the distribution of the displacements 
\begin{equation}
	\alpha_2(t) = \frac{3 \left\langle {\Delta r}^4(t) \right\rangle}{5 {\left\langle {\Delta r}^2(t) \right\rangle}^2}-1
	\label{eq:ng}
\end{equation}
At a given volume fraction, $\alpha_2(t)$ peaks at the characteristic time scale of the dynamic heterogeneity $t^{dh}(\phi)$. The volume fraction dependence of both $\tau_\alpha$ and $t^{dh}$ is plotted in Fig.~M\ref{fig:vft}b. We notice that $t^{dh}$ corresponds to the end of plateau, always shorter than $\tau_\alpha$.
...
\end{center}
	\caption{\textbf{Sizing} of our colloids. \textbf{a} Size distribution estimated \emph{in situ} (dashed line) by our multiscale algorithm ($\sim 1.8\times 10^6$ instantaneous sizing). Comparison with the size distribution estimated from \textsc{sem} of only $140$ dry particles (steps) is possible once controlled for $23\%$ of swelling (full line). \textbf{b} First peak of the radial distribution function with (full line) and without (dashed) the size data. Taking into account the measured sizes rectifies the effect of the polydispersity: the peak is thinner and higher.}
	\label{fig:sizing}
\end{figure}
\clearpage
\begin{figure}
