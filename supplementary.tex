...
\begin{equation}
	...
	\label{eq:scale_dil}
\end{equation}
where $\alpha = 2^{1/n}$ is the step of the discrete scale space. Eq.~(S\ref{eq:scale_dil}) allows to translate the $n$-dependent $\sigma^*$ to the parameter-free real radius of the particle, $R$.} The error on $R$ does depend on the number of subdivisions.
