...
Figure~S\ref{fig:sizing}a shows the size distribution of the suspensions investigated in the main text. Once controlled for a solvent swelling of $23\%$ in radius, our \emph{in situ} measurements compare well with the size distribution obtained from scanning electron microscopy (\textsc{sem}) of the same dry colloids. However we note that our method finds relatively more small particles than the \textsc{sem}. A careful comparison between confocal images and algorithm output suggest that this is not an artefact: the objects detected as small ($\unit{1.5}{\micro\metre}$ to $\unit{3}{\micro\metre}$) do correspond to small spots (3-6 pixels width) in the confocal images. Thus we conclude that the relative lack of small particles in the \textsc{sem} measurement is probably due to bad sampling (the sizes of only $140$ particles were measured). 
