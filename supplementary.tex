...
\end{equation}
with $\alpha=2^{1/n}$. Here it is clear that minimizing $DoG(x, \alpha)$ with respect to $x$ yields a value $x^*$ that depends only on $\alpha$. 
Practically $x^*$ is obtained by a polynomial fit for discrete data $x_j$. 
This means the optimal value of $\sigma$, $\sigma^*$, is proportional to $R$.  Both $\sigma^*$ and the proportionality constant are function of $n$, but $R$ is an absolute value independent of the way we measure it. Formally, we have
\begin{equation}
	R = \sigma^* \sqrt{\frac{6\ln \alpha}{1-\alpha^{-2}}}, 
	\label{eq:scale_dil}
\end{equation}
that allows to translate the $n$-dependent $\sigma^*$ to the parameter-free real radius of the particle, $R$.} The error on $R$ does depend on the number of subdivisions.
